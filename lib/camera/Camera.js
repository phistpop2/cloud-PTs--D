...
Camera = function(){
    ...
    return {
        ...
        lookFacade: function(q){
            mQuaternion = q;
            quaternMatrix4 = mQuaternion.getMatrix();
            navi.refresh( mQuaternion.getMatrixQuery() );
        },
        ...
        //rotate world....
        rotateWorldX: function( dx ){
            var xq = quaternion().fromAxisAngle( vector3(-1, 0, 0), dx/2 );
            mQuaternion = mQuaternion.crossBy( xq );
            quaternMatrix4 = mQuaternion.getMatrix();
            var bias = vector3( 0, $('#workSpace').height()/2, 0);
            eye.sum( bias );
            eye = getRotateVector( eye, xq );
            eye.sub( bias );
            transformed = true;

        },
        rotateWorldY: function( dy ){
            var yq = quaternion().fromAxisAngle( vector3(0, 1, 0), dy/2 );
            mQuaternion = mQuaternion.crossBy( yq.getInverse() );
            quaternMatrix4 = mQuaternion.getMatrix();
            var bias = vector3( -$('#workSpace').width()/2,0, 0 );
            eye.sum( bias );
            eye = getRotateVector( eye, yq );
            eye.sub( bias );
            transformed = true;
        },
        rotateWorldZ: function( dz ){
            var zq = quaternion().fromAxisAngle( vector3(0, 0, 1), dz/2 );
            mQuaternion = mQuaternion.crossBy( zq );
            quaternMatrix4 = mQuaternion.getMatrix();
            var bias = vector3( -$('#workSpace').width()/2,  $('#workSpace').height()/2, 0 );
            eye.sum( bias );
            eye = getRotateVector( eye, zq );
            eye.sub( bias );
            transformed = true;
        },

        //rotate camera ...need to translate!    perspective 600px :D
        rotateX: function( dx ){
            var xq = quaternion().fromAxisAngle( vector3(-1, 0, 0), dx/2 );
            mQuaternion = mQuaternion.crossBy( xq );
            quaternMatrix4 = mQuaternion.getMatrix();
            var bias = vector3( 0, $('#workSpace').height()/2, -600 );
            eye.sum( bias );
            eye = getRotateVector( eye, xq );
            eye.sub( bias );
            transformed = true;
        ...
        rotateY: function( dy ){
            ...
            mQuaternion = mQuaternion.crossBy( yq.getInverse() );
            quaternMatrix4 = mQuaternion.getMatrix();
            var bias = vector3( -$('#workSpace').width()/2,0, -600 );
            eye.sum( bias );
            eye = getRotateVector( eye, yq );
            eye.sub( bias );
            transformed = true;
        ...
        rotateZ: function( dz ){
            ...
            mQuaternion = mQuaternion.crossBy( zq );
            quaternMatrix4 = mQuaternion.getMatrix();
            var bias = vector3( -$('#workSpace').width()/2,  $('#workSpace').height()/2, -600 );
            eye.sum( bias );
            eye = getRotateVector( eye, zq );
            eye.sub( bias );
            transformed = true;
