    ...
    return {
        ...
        isCurrentScreen : function( w, h ){
            ...
            for( var i = 0; i < points.length; i++ ){
                if( ( points[i].x > 0 && points[i].x < right ) && ( points[i].y > 0 && points[i].y < bottom && points[i].z > -500 ) ){
                    return true;
                }
            }
            return false;
        ...
        zoomFacade: function(width,height){
            ...
            var bias = vector3( parseInt($('#workSpace').width())/2, -parseInt($('#workSpace').height())/2, 0 );
