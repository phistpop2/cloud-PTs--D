...
ObjectController = function(camera){
    ...
    return {
        ...
        getAbsoluteXY : function(src, w, h){
            ...
            var newY = ((newH-h)/2+ov.y)*(height/newH);
            var newX = ((newW-w)/2+ov.x)*(width/newW);
        ...
        getAbsoluteXYSub : function(src, w, h){
            var ov = this.getTranslateBiasToCurrentWorld( src, w, h );
            var height = parseInt( $('#workSpace').height() );
            var width = parseInt( $('#workSpace').width() );
            var perspective = parseInt( $('#workSpace').css('-webkit-perspective') );
            var z = -ov.z;
            ...
            var newH = (height/2 + z*Math.tan(radian/2))*2;
            var newW = (width/2 + z*Math.tan(radian/2))*2;
            var newW2 = width*(perspective+z)/(perspective);

            console.debug( newW, newW2, (height*z)/(newH-height) );

            var newHGap = ((newH/2+ov.y-h/2)*(height/newH) );
            var newWGap = ((newW/2+ov.x-w/2)*(width/newW) );
            var newWGap2 = ((newW2/2+ov.x-w/2)*(width/newW2) );

            console.debug( '!!!!!!!!', newHGap, newWGap, newWGap2 );
        },
        ...
        getCetnerPosition : function(w, h){
            var width = parseInt( $('#workSpace').width() );
            var height = parseInt( $('#workSpace').height() );
            ...
            /* in the world*/
            //get center vector ***
            var objectInnerCenter = vector3(0,0,0);
            var q = parent.getQuaternion().getInverse().crossBy( mQuaternion.getInverse() );
            ...
            revisedY.scalar( -h/2 );
            revisedZ.scalar(0);
            objectInnerCenter.sum( revisedX ).sum( revisedY ).sum( revisedZ );

            //var objectPosition = eye.clone();
            var objectCenter = vector3().add( pos, objectInnerCenter );
            var initScreenPosition = vector3( (width-w)/2, (h-height)/2, 0 );
            objectCenter.sum( initScreenPosition );
            ...
            revisedX = getInverseVector( vector3(1, 0, 0) );
            revisedY = getInverseVector( vector3(0, -1, 0) );
            revisedZ = getInverseVector( vector3(0, 0, 1) );
            revisedX.scalar( objectCenter.getX() );
            revisedY.scalar( objectCenter.getY() );
            revisedZ.scalar( objectCenter.getZ() );
            //objectCenter = revisedX.sum( revisedY ).sum( revisedZ );
            //objectCenter.sum( parent.getLocation().clone() );
            var edge = this.getAbsoluteEdges( objectCenter, w, h );
            ...
            for( var i = 0; i < 1; i++ ){
                ...
                var newY = ((newH)/2-(height/2+edge[i].getY()))*(height/newH);
                var newX = ((newW)/2-(width/2-edge[i].getX()))*(width/newW);
                var newHGap = ((newH/2-(height/2+edge[i].getY()))*(height/newH) );
                //var newWGap = ((newW/2+ov.x-w/2)*(width/newW) );
                console.debug( 'edge['+i+']', parseInt( newY ), parseInt( newX ),(height/newH),newH, h );
            }
        ...
        getAbsoluteEdges : function(src, w,h){
            ...
            ov.sum( revisedX ).sum( revisedY ).sum( revisedZ );
            ...
            revisedX = getRotateVector( vector3(1, 0, 0), mQuaternion.getInverse() );
            revisedY = getRotateVector( vector3(0, -1, 0), mQuaternion.getInverse() );
            revisedZ = getRotateVector( vector3(0, 0, 1), mQuaternion.getInverse() );


            //nw
            var rx = revisedX.getScalar( -w/2  );
            var ry = revisedY.getScalar( h/2 );
            var rz = revisedZ.getScalar( 0 );
            edge[0].sum( rx ).sum( ry ).sum( rz );

            //ne
            rx = revisedX.getScalar( w/2  );
            ry = revisedY.getScalar( h/2 );
            ...
            for( var i = 0; i < edge.length ; i++ ){
                ...
                var pos = vector3(0,0,0);
                var revisedVector = getInverseVector( vector3(1, 0, 0) );
                pos.sum(  revisedVector.getScalar( edge[i].getX() ) );


                revisedVector = getInverseVector( vector3(0, -1, 0) );
                pos.sum( revisedVector.getScalar( edge[i].getY() ) );

                revisedVector = getInverseVector( vector3(0, 0, 1) );
                pos.sum( revisedVector.getScalar( edge[i].getZ() ) );
                result[i] = pos;
                //result[i].setY( -result[i].getY() );
            }
            return result;
        },
